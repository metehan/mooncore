...
function App() {
  ...
  // Fetch custom pages from API and build merged menu
  useEffect(() => {
    async function fetchCustomPages() {
      try {
        ...
        // Auto-select first custom page if currently showing a custom page that no longer exists
        // (handled by the setPage call below)
      } catch (e) {
        setCustomPages([]);
      }
    }
  ...
  // Build menu: default items + custom pages with conflict resolution
  const defaultMenuIds = new Set(MENU.map(m => m.id.toLowerCase()));
  const menuItems = MENU.map(m => ({ id: m.id, label: m.label, isDefault: true }));
  customPages.forEach(cp => {
    const lower = cp.name.toLowerCase();
    const id = defaultMenuIds.has(lower) ? `app_${cp.name}` : cp.name;
    const label = defaultMenuIds.has(lower) ? `App ${cp.name}` : cp.name;
    const icon = cp.definition?.icon || null;
    menuItems.push({ id, label, isDefault: false, pageName: cp.name, icon });
  });

  // Determine if current page is a custom page and get its pageName
  const isCustomPage = !defaultMenuIds.has(page);
  const activeCustomPage = isCustomPage ? customPages.find(cp => cp.name === page || `app_${cp.name}` === page) : null;
...
<div class="flex flex-1 overflow-hidden h-screen">
    ...
    <nav class="flex-1 py-2">
      ${menuItems.map(m => html`
        <button key=${m.id}
          ...
          <span class="w-5 text-center flex items-center justify-center">${m.icon ? html`<span style="width:20px;height:20px;display:inline-flex;color:inherit" dangerouslySetInnerHTML=${{ __html: m.icon }} />` : (ICONS[m.id] || ICONS['custom-pages'])}</span>
          ${m.label}
        </button>
      `)}
    </nav>
  ...
  <div class="flex-1 overflow-y-auto min-w-0">
    ...
    ${isCustomPage && activeCustomPage && html`<${CustomPageView} pageName=${page} pageData=${activeCustomPage} />`}
  </div>
