  ...
  <div class="h-full flex min-h-0">
    <div class="w-72 border-r border-gray-800 bg-gray-900/50 shrink-0 flex flex-col">
      ...
      <div class="flex-1 overflow-y-auto">
        ${guides.length === 0
      ? html`<div class="p-5 text-sm text-gray-600">No guides found. Add <code>.md</code> files to your project's <code>guides/</code> directory.</div>`
      : guides.map(g => html`
            <button key=${g.file}
              class="group w-full text-left px-4 py-3 bg-transparent border-x-0 border-t-0 border-b border-gray-800/30 cursor-pointer transition-colors
                ${selectedFile === g.file ? 'bg-violet-500/10' : 'hover:bg-gray-800/40'}"
              onClick=${() => loadGuide(g)}>
              <div class="flex items-start gap-3">
                <span class="text-base ${selectedFile === g.file ? 'text-violet-400' : 'text-gray-500 group-hover:text-gray-300'}">\uD83D\uDCD6</span>
                <div class="min-w-0 flex-1">
                  <div class="text-sm ${selectedFile === g.file ? 'text-violet-300' : 'text-gray-200'} truncate">${g.name}</div>
                  <div class="text-xs text-gray-600 truncate">${g.file}</div>
                </div>
                <span class="text-[10px] uppercase tracking-wider text-gray-700 group-hover:text-gray-500">Open</span>
              </div>
            </button>
          `)
    }
    ...
    <div class="flex-1 min-w-0 flex flex-col">
      <div class="px-4 py-3 border-b border-gray-800 flex items-center gap-3 bg-gray-900/50 shrink-0">
        <div class="min-w-0 flex-1">
          <div class="text-xs font-semibold text-gray-400 uppercase tracking-wider truncate">${selected || 'Guides'}</div>
          ${selectedFile && html`<div class="text-xs text-gray-600 truncate mt-0.5">${selectedFile}</div>`}
        </div>
