...
function ApiPage({ pendingLoad, onConsumePendingLoad, onToEval }) {
  ...
  const [sortKey, setSortKey] = useState('role'); // 'role' | 'name'
  ...
  const filteredActions = useMemo(() => {
    ...
    return [...filtered].sort((a, b) => {
      const roleA = a.public ? '' : (a.roles || []).join(' ');
      const roleB = b.public ? '' : (b.roles || []).join(' ');
      if (sortKey === 'role') {
  ...
  }, [actions, filterText, filterRole, sortKey, sortDir]);
  ...
  const sortBtn = key => `px-2 py-0.5 text-xs border-none cursor-pointer rounded transition-colors
    ${sortKey === key ? 'bg-violet-500/20 text-violet-300' : 'bg-transparent text-gray-600 hover:text-gray-400'}`;
  ...
  function replayFromLogs(data) {
    ...
    setResult(data.response != null ? data.response : null);
  }
...
<div class="h-full flex overflow-hidden">
  ...
  <div class="flex flex-col border-r border-gray-800 overflow-hidden" style="width:38%;min-width:220px">
    ...
    ${leftTab === 'actions' && html`
      ...
      <div class="px-3 py-1.5 border-b border-gray-800 flex items-center bg-gray-900/30">
        <button class="text-xs font-semibold text-gray-400 border-none cursor-pointer bg-transparent"
          onClick=${() => { if (sortKey === 'name') setSortDir(d => d === 'asc' ? 'desc' : 'asc'); else { setSortKey('name'); setSortDir('asc'); } }}>
          Action ${sortKey === 'name' ? (sortDir === 'asc' ? '↑' : '↓') : ''}
        </button>
        <button class="ml-auto text-xs font-semibold text-gray-400 border-none cursor-pointer bg-transparent"
          onClick=${() => { if (sortKey === 'role') setSortDir(d => d === 'asc' ? 'desc' : 'asc'); else { setSortKey('role'); setSortDir('asc'); } }}>
          Roles ${sortKey === 'role' ? (sortDir === 'asc' ? '↑' : '↓') : ''}
        </button>
      </div>
        ...
        <div class="p-5 space-y-4">
          ...
          ${result != null && html`
            <div class="rounded border ${result.error || result.ok === false ? 'border-red-900/50' : 'border-gray-800'} overflow-hidden">
              <div class="px-3 py-1.5 border-b ${result.error || result.ok === false ? 'border-red-900/50 bg-red-900/10' : 'border-gray-800 bg-gray-900/50'} flex items-center gap-2">
                ...
                <div class="flex items-center gap-1 ml-auto">
                  <button class="px-2 py-0.5 text-xs text-gray-500 hover:text-gray-300 bg-transparent border-none cursor-pointer"
                    onClick=${() => navigator.clipboard.writeText(JSON.stringify(result, null, 2))}>Copy JSON</button>
                  <button class="px-2 py-0.5 text-xs text-gray-500 hover:text-gray-300 bg-transparent border-none cursor-pointer"
                    onClick=${() => navigator.clipboard.writeText(jsonToElixir(result))}>Copy Map</button>
                </div>
              </div>
