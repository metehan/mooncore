...
function DashOverview({ data, history, cpuPct, avgCpu }) {
  ...
  <!-- Devtools warning -->
  ${!data.dev_tools_allowlist_set && html`
    <div class="${wrapperClass}" style=${wrapperStyle}>
      <div class="flex items-start gap-3">
        <span class="${warnIconClass} text-lg leading-none mt-0.5" aria-hidden>⚠</span>
        <div class="space-y-2">
          <div class="text-sm font-semibold ${headerClass}">IP allowlist not configured</div>
          <div class="text-xs leading-relaxed">The application has not configured <code>dev_tools_allowed_ips</code>. Without an IP allowlist, dev tools will accept requests from any IP.</div>
          <div class="text-xs leading-relaxed">Add an allowlist in your config (example):</div>
          <pre class="mt-2 text-xs font-mono bg-gray-800/40 p-2 rounded"><code>config :mooncore, dev_tools_allowed_ips: ["127.0.0.1", "192.168.1.0/24"]</code></pre>
        </div>
      </div>
    </div>
  `}
